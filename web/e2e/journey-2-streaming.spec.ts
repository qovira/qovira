...
test("streaming → tool chip → entity card → reminders view", async ({ page }) => {
  // Land on the chat surface with the SSE connection open before sending, so the
  // turn's streaming deltas and tool events are not raced (see gotoConnected).
  ...
  // The in-flight assistant bubble renders streaming text (textDelta chunks).
  // Wait for any partial text to appear before asserting the entity card.
  await expect(page.getByText("Sure, I'll")).toBeVisible({ timeout: 15_000 });

  // ── Entity card appears ───────────────────────────────────────────────────
  // After the turn completes the tool chip resolves to an <a> entity card
  // with aria-label "Reminder: Buy milk". The label pattern matches the
  // ToolCallChip component: "{entity_type}: {title}".
