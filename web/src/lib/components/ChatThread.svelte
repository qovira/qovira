<script lang="ts">
  ...
  //
  // Rendered by both +page.svelte (the live chat route) and Storybook stories.
  // Reads directly from the conversation, tool-calls, and confirmations stores so
  // stories exercise the actual rendering paths without bootstrapping a full SvelteKit
  // page.
  //
  // Security: assistant text flows through renderSafeMarkdown() (marked → DOMPurify)
  // before {@html}. User messages are rendered as escaped {content} — never {@html}.
  // This is the ONLY place the {@html} rendering path exists; +page.svelte delegates
  // here entirely.
