...
// Holds the live list of reminders as patched by SSE events + REST reconcile. The SSE router calls upsertReminder /
// removeReminder; the REST reconcile calls upsertReminder in bulk. The layout resets the store on logout / 401
// teardown.
 ...
 * If a reminder with the same id already exists it is replaced; otherwise the new item is appended. Called by the SSE
 * router on reminder.* events and by the REST reconcile on reconnect.
 */
...
/**
 * Remove the reminder with the given id. No-op when the id is not found. Called by the SSE router on reminder.deleted.
 */
...
/**
 * Replace the entire reminders list with the given items. Called by the REST reconcile on reconnect to resync from
 * server truth.
 */
...
/**
 * Reset the reminders store to empty. Call on logout or 401 teardown. Safe when already empty.
 */
