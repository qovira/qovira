...
// The pre-paint theme boot snippet is inlined verbatim in app.html (a dynamic import runs too late
// and reintroduces the flash). @qovira/theme is the source of truth, so this guards against the
// inlined copy drifting from the upstream `boot` export — collapsing whitespace so Prettier's
// indentation in app.html is irrelevant while any change to a key, value, or the decision tree
// fails.
...
describe("pre-paint theme boot snippet", () => {
  ...
  it("runs before any stylesheet so first paint has no theme flash", () => {
    ...
    // SvelteKit emits the app's stylesheet links at %sveltekit.head%; the boot script must precede
    // it (and there must be no earlier stylesheet link).
