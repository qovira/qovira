...
//
// In-package test so it can drive the unexported newChatter directly, keeping the e2e build's public API free of
// test-only helpers.
//
...
// TestNewChatterSelector_WithEnvSet verifies that when QOVIRA_E2E_SCRIPT_PATH is set to a valid fixture file the
// selector returns a *ScriptedChatter. Cannot use t.Parallel because t.Setenv is not compatible with t.Parallel.
...
// TestNewChatterSelector_WithoutEnv verifies that when QOVIRA_E2E_SCRIPT_PATH is not set the selector returns a
// *gateway.Gateway (the real gateway). Cannot use t.Parallel because t.Setenv is not compatible with t.Parallel.
