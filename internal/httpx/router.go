...
// Router wraps a *http.ServeMux and is the mount point for module routes.
// Each module receives a *Router in its Routes method and registers its own
// fully-qualified patterns, by convention under /api/v1/... (e.g.
// "GET /api/v1/reminders"). Go's ServeMux resolves by specificity, so a
// concrete module pattern like "GET /api/v1/reminders" always beats the
// "/api/v1/{path...}" catch-all registered by NewServer, regardless of
// registration order.
//
// NewServer registers its own fixed routes (healthz, API catch-all, /events,
// /) after module routes are mounted; both sets share the same underlying mux.
type Router struct {
	mux *http.ServeMux
}

// NewRouter constructs and returns a ready-to-use *Router backed by a fresh
// *http.ServeMux.
