...
// DeleteUserData deletes a user_data row by id, scoped to the bound user. Returns an error if the scope is invalid.
...
func (sq *ScopedQueries) GetProfile(ctx context.Context) (db.User, error) {
	...
	if sq.scope.UserID() == "" {
		return db.User{}, fmt.Errorf("GetProfile: %w", ErrEmptyUserID)
	}
	user, err := sq.readQ.GetUserByID(ctx, sq.scope.UserID())
	if err != nil {
		return db.User{}, fmt.Errorf("GetProfile: %w", err)
	}
	return user, nil
}
