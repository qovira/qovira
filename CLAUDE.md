...
**`internal/gateway` is the model-gateway layer.** `Gateway` resolves capability roles (chat, embeddings, vision, …) to AI endpoint coordinates and forwards requests to the configured upstream. Configuration lives in the `"model_gateway"` settings namespace and is read-through on every call. `Chat(ctx, req)` returns an `iter.Seq2[Chunk, error]` iterator with a two-phase error model (setup error vs. per-yield error). The **resilience layer** (`resilience.go`) wraps every `Chat` call with three timeout controls — first-token timeout (default 45 s), idle timeout (default 30 s), and ctx propagation — plus a pre-first-token retry loop (default 3 attempts, jittered backoff) that retries on 5xx, network failures, and 429 (honouring `Retry-After`). Generic 4xx, `ErrContextLength`, `ErrAuth`, and `ErrModelNotFound` are never retried; 451 Unavailable For Legal Reasons is retried only when the `retryLegalUnavailable` setting is `true`. All retry/timeout parameters are tunable via `Gateway.resilienceCfg` (`ResilienceConfig`); the `sleepFn` field is injectable for deterministic testing with zero real sleep. The internal HTTP client carries no wall-clock timeout — timeouts are managed entirely by the resilience layer via derived cancellable contexts. `Probe(ctx, role)` empirically checks an endpoint: step 1 does GET `/v1/models` (reachability + model presence); step 2 (chat role only) sends a minimal streamed request with a forced dummy tool call to verify streaming and native tool calling. It returns a `ProbeResult` struct with independent boolean fields — `Reachable`, `ModelServed`, `Streaming`, `ToolCalling` — and an `Err` for the first failure encountered.
